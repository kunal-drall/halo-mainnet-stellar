 ...
 *
 * Run: cd app && npx ts-node --compiler-options '{"module":"commonjs"}' ../scripts/demo-workflow-v7.ts
 */
...
async function main() {
  ...
  console.log("  Halo Protocol — v7 Workflow (8 users, 3 circles)");
  console.log("═".repeat(60));
  ...
  const wallets: any[] = USER_NAMES.map((name, i) => {
    ...
    const uid = crypto.createHash("sha256").update(`halo-v7-${name}-${Date.now()}-${i}`).digest();
  ...
  fs.writeFileSync(path.join(WALLETS_DIR, "v7-wallets.json"),
  ...
  // Step 4: 3 Circles
  // Circle 1: Aarav (0), Zara (1), Rehan (2)
  await runCircle(server, "Kolkata Finance Club", "KolFin", wallets[0], [wallets[1], wallets[2]], 55);
  await sleep(3000);

  // Circle 2: Tanya (3), Dev (4), Sneha (5)
  await runCircle(server, "Chennai Savers Ring", "ChenSave", wallets[3], [wallets[4], wallets[5]], 80);
  await sleep(3000);

  // Circle 3: Aryan (6), Kavya (7), Zara (1) — cross-circle member
  await runCircle(server, "Ahmedabad Growth Fund", "AhmGrow", wallets[6], [wallets[7], wallets[1]], 65);
  ...
  saveResults(wallets);
  console.log("\n" + "═".repeat(60));
  console.log("  ✓ v7 workflow complete!");
  console.log("═".repeat(60));
}
...
function saveResults(wallets: any[]) {
  ...
  fs.writeFileSync(path.join(WALLETS_DIR, "v7-results.json"),
