/**
 * Halo Protocol — On-Chain Demo Workflow v2
 *
 * Adds 5 new users with Indian names + 2 new circles to the testnet.
 * Merges results into existing explorer-data.json.
 *
 * Run: cd app && npx ts-node --compiler-options '{"module":"commonjs"}' ../scripts/demo-workflow-v2.ts
 */
...
async function main() {
  ...
  console.log("  Halo Protocol — v2 Workflow (5 users, 2 circles)");
  console.log("═".repeat(60));
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const seed = `halo-v2-${USER_NAMES[i]}-${Date.now()}-${i}`;
  ...
  fs.writeFileSync(
    path.join(WALLETS_DIR, "v2-wallets.json"),
  ...
  // Step 4: Circle 1 — Arjun (creator), Siddharth, Rohan
  await runCircle(
    server,
    "Sharma Savings Club",
    "SharmaSave",
    wallets[0], // Arjun Sharma
    [wallets[1], wallets[2]], // Siddharth Mehta, Rohan Verma
    50
  );

  await sleep(3000);

  // Step 5: Circle 2 — Ishaan (creator), Priya, Siddharth
  await runCircle(
    server,
    "Delhi Growth Circle",
    "DelhiGrow",
    wallets[3], // Ishaan Gupta
    [wallets[4], wallets[1]], // Priya Nair, Siddharth Mehta (in 2 circles)
    75
  );
  ...
  console.log("\n" + "═".repeat(60));
  console.log("  ✓ v2 workflow complete!");
  console.log("═".repeat(60));
}
...
function saveResults(wallets: any[]) {
  ...
  // Private results
  fs.writeFileSync(
    path.join(WALLETS_DIR, "v2-results.json"),
