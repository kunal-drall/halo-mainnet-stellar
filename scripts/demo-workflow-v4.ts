/**
 * Halo Protocol — On-Chain Demo Workflow v4
 *
 * Adds 5 more users with Indian names + 2 more circles to the testnet.
 * Merges results into existing explorer-data.json.
 *
 * Run: cd app && npx ts-node --compiler-options '{"module":"commonjs"}' ../scripts/demo-workflow-v4.ts
 */
...
async function main() {
  ...
  console.log("  Halo Protocol — v4 Workflow (5 users, 2 circles)");
  console.log("═".repeat(60));
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const seed = `halo-v4-${USER_NAMES[i]}-${Date.now()}-${i}`;
  ...
  fs.writeFileSync(
    path.join(WALLETS_DIR, "v4-wallets.json"),
  ...
  await runCircle(
    server,
    "Bangalore Tech Circle",
    "BangTech",
    wallets[0], // Ananya Singh
    [wallets[1], wallets[2]], // Karan Bajaj, Vikram Nair
    70
  );
  ...
  await runCircle(
    server,
    "Hyderabad Savings Club",
    "HydSave",
    wallets[3], // Divya Menon
    [wallets[4], wallets[1]], // Rahul Khanna, Karan Bajaj (in 2 circles)
    55
  );
  ...
  console.log("\n" + "═".repeat(60));
  console.log("  ✓ v4 workflow complete!");
  console.log("═".repeat(60));
}
...
function saveResults(wallets: any[]) {
  ...
  // Private results
  fs.writeFileSync(
    path.join(WALLETS_DIR, "v4-results.json"),
