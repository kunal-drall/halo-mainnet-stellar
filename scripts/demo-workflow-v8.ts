 ...
 *
 * Run: cd app && npx ts-node --compiler-options '{"module":"commonjs"}' ../scripts/demo-workflow-v8.ts
 */
...
async function main() {
  ...
  console.log("  Halo Protocol — v8 Workflow (8 users, 3 circles)");
  console.log("═".repeat(60));
  ...
  const wallets: any[] = USER_NAMES.map((name, i) => {
    ...
    const uid = crypto.createHash("sha256").update(`halo-v8-${name}-${Date.now()}-${i}`).digest();
  ...
  fs.writeFileSync(path.join(WALLETS_DIR, "v8-wallets.json"),
  ...
  // Step 4: 3 Circles
  // Circle 1: Rohit (0), Priyanka (1), Aakash (2)
  await runCircle(server, "Surat Savings Circle", "SuratSave", wallets[0], [wallets[1], wallets[2]], 70);
  await sleep(3000);

  // Circle 2: Nandini (3), Siddharth (4), Ritika (5)
  await runCircle(server, "Lucknow Finance Club", "LucknowFin", wallets[3], [wallets[4], wallets[5]], 50);
  await sleep(3000);

  // Circle 3: Vivek (6), Anushka (7), Priyanka (1) — cross-circle member
  await runCircle(server, "Indore Growth Fund", "IndoreGrow", wallets[6], [wallets[7], wallets[1]], 85);
  ...
  saveResults(wallets);
  console.log("\n" + "═".repeat(60));
  console.log("  ✓ v8 workflow complete!");
  console.log("═".repeat(60));
}
...
function saveResults(wallets: any[]) {
  ...
  fs.writeFileSync(path.join(WALLETS_DIR, "v8-results.json"),
