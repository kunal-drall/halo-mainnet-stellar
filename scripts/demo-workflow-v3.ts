/**
 * Halo Protocol — On-Chain Demo Workflow v3
 *
 * Adds 5 more users with Indian names + 2 more circles to the testnet.
 * Merges results into existing explorer-data.json.
 *
 * Run: cd app && npx ts-node --compiler-options '{"module":"commonjs"}' ../scripts/demo-workflow-v3.ts
 */
...
async function main() {
  ...
  console.log("  Halo Protocol — v3 Workflow (5 users, 2 circles)");
  console.log("═".repeat(60));
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const seed = `halo-v3-${USER_NAMES[i]}-${Date.now()}-${i}`;
  ...
  fs.writeFileSync(
    path.join(WALLETS_DIR, "v3-wallets.json"),
  ...
  // Step 4: Circle 1 — Vaibhav (creator), Lakshay, Harshit
  await runCircle(
    server,
    "Mumbai Savers Circle",
    "MumbaiSave",
    wallets[0], // Vaibhav Tiwari
    [wallets[1], wallets[2]], // Lakshay Kapoor, Harshit Agarwal
    60
  );

  await sleep(3000);

  // Step 5: Circle 2 — Neha (creator), Riya, Lakshay
  await runCircle(
    server,
    "Pune Wealth Circle",
    "PuneWealth",
    wallets[3], // Neha Joshi
    [wallets[4], wallets[1]], // Riya Malhotra, Lakshay Kapoor (in 2 circles)
    80
  );
  ...
  console.log("\n" + "═".repeat(60));
  console.log("  ✓ v3 workflow complete!");
  console.log("═".repeat(60));
}
...
function saveResults(wallets: any[]) {
  ...
  // Private results
  fs.writeFileSync(
    path.join(WALLETS_DIR, "v3-results.json"),
